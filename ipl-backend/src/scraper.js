...
async function scrapeIPLData() {
  ...
  try {
          ...
          if (team1 && team2 && team1 !== team2) {
            matches.push({
              ...
              homeTeam: { 
                ...
                name: team1, 
                shortName: team1.split(' ').map(w => w[0]).join('') 
              },
              awayTeam: { 
                id: team2.toLowerCase().replace(/\s+/g, ''), 
                name: team2, 
                shortName: team2.split(' ').map(w => w[0]).join('') 
              },
    ...
    const liveMatch = scheduleData.find(match => match.status === 'live');
    const upcomingMatches = scheduleData.filter(match => match.status === 'upcoming').slice(0, 5);
